class Resto {
    ...
    public synchronized void serveCustomer(String cashierName) { // Method synchronize agar thread mengantri
        if (chickenStock > 0) {
            try { Thread.sleep(10); } catch (InterruptedException e) {} // Jeda sangat sebentar. Kalo diinterrupt ada penanganan
    ...
    public int getRemainingStock() { // Mengembalikan nilai stok ayam
        return chickenStock;
    }
}
...
public class RestoSimulasi {
    public static void main(String[] args) throws InterruptedException {
        ...
        // // Membuat tugas (Runnable) di mana setiap kasir akan mencoba menjual 40 ayam melalui perulangan (loop)
        Runnable task = () -> {
            for (int i = 0; i < 40; i++) {
